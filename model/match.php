...
class MatchStatus
{
    ...
    public static function update()
    {
        ...
        $second_select = $dbc->prepare("SELECT user_id FROM users WHERE partner_id IS NULL AND user_id <> ?");
        $second_select->bind_param("i", $first_id);
        $second_select->execute();
        $second_select->bind_result($second_id);

        $first_update = $dbc->prepare("UPDATE users SET partner_id=? WHERE user_id=?");
        $first_update->bind_param("ii", $second_id, $first_id);
        $first_update->execute();

        $second_update = $dbc->prepare("UPDATE users SET partner_id=? WHERE user_id=?");
        $second_update->bind_param("ii", $first_id, $second_id);
        $second_update->execute();

        $pair_insert = $dbc->prepare("INSERT INTO pairs (user_id1, user_id2) VALUES(?,?)");
